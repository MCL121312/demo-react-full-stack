...
router.post('/articles', async (c) => {
  ...
  if (!result.success) {
    return c.json({ message: '请求参数校验失败', errors: result.error.issues }, 400);
  }
  const { title, content, tier } = result.data;
  const stmt = db.prepare(
    'INSERT INTO articles (title, content, tier) VALUES (?, ?, ?) RETURNING *',
  );
...
router.put('/articles/:id', async (c) => {
  ...
  const { title, content, tier } = result.data;
  const stmt = db.prepare(
    'UPDATE articles SET title = ?, content = ?, tier = ? WHERE id = ? RETURNING *',
  );
